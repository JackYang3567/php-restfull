...
{include file="../layouts/header.tpl" title='GK数据API'}
  ...
  <body>
    ...
    <div class="x-body">
      <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so">
          ...
          <input type="text" name="username"  placeholder="请输入支付方式名" autocomplete="off" class="layui-input">
          <button class="layui-btn"  lay-submit="" lay-filter="sreach"><i class="layui-icon">&#xe615;</i></button>
        </form>
      </div>
      <xblock>
        <button class="layui-btn layui-btn-danger" onclick="delAll()"><i class="layui-icon"></i>批量删除</button>
        <button class="layui-btn" onclick="x_admin_show('添加支付方式','/backend/index.php/Admin/NewPaymethod',600,400)"><i class="layui-icon"></i>添加</button>
      ...
      <table class="layui-table x-admin">
        ...
        <div style="display:none">
          ...
          <input type="hidden" id="api" value="/paymethod/index.php/Paymethod">
      </div>
    ...
    //load info
     function getPaymethodlist(reloadUrl,step=''){
        $.ajax({
            url:reloadUrl,
            type:"GET",
            data:'',
     ...
     function pagein(){
       ...
       trStr +=`<tr>
           ...
           trStr += status+` <a title="编辑"  onclick="x_admin_show('编辑支付方式','/backend/index.php/Admin/EditPaymethod?id=`+data[k].Id +`',600,400)" href="javascript:;">
              ...
              <a title="删除" onclick="paymethod_del(this,'`+data[k].Id +`')" href="javascript:;">
                <i class="layui-icon">&#xe640;</i>
              </a>
            </td>
          </tr>`
